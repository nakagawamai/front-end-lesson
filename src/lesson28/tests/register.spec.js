...
test('If all inputs are correct and the checkbox agreeing to the terms of use is checked, the user will be registered', async ({ page }) => {
    await page.goto('http://localhost:3000/lesson28/register.html');
    ...
    await page.screenshot({ path: "./playwright/screenshots/register/register-done.png" });
});
...
test('if the email address is already registered, display an error message', async ({ page }) => {
    ...
    await page.screenshot({ path: "./playwright/screenshots/register/register-email-error.png" });
});
